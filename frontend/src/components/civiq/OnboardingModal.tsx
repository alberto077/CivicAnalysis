    ...
    >
      <div className="w-full max-w-lg rounded-2xl bg-white p-8 shadow-[0_20px_60px_-15px_rgba(26,54,93,0.4)] animate-in fade-in zoom-in duration-300">
        <div className="mb-6 flex items-center justify-between">
          <p className="font-work-sans text-sm font-semibold tracking-widest text-(--accent-mid) uppercase">
            Step {step} of 5
          </p>
          <p className="font-work-sans text-xs font-normal text-(--muted)">Privacy: Saved only to your browser</p>
        </div>

        {step === 1 && (
          <div className="space-y-6">
            <h2 className="font-limelight text-2xl font-bold text-foreground tracking-tight">Where do you live?</h2>
              ...
              {BOROUGHS.map((b) => (
                <button
                  ...
                  className={`rounded-xl border p-3 text-left font-work-sans text-sm font-medium transition ${data.borough === b
                    ...
                    : "border-(--border) bg-background text-foreground hover:border-(--accent-mid)"
                    }`}
                >
                  {b}
                </button>
        ...
        {step === 2 && (
          <div className="space-y-6">
            <h2 className="font-limelight text-2xl font-bold text-foreground tracking-tight">Housing & Income</h2>
            <div className="space-y-4">
              <div>
                <label className="font-work-sans mb-2 block text-sm font-medium text-(--muted)">Housing Status</label>
                <div className="flex flex-wrap gap-2">
                  {HOUSING.map((h) => (
                    <button
                      key={h}
                      onClick={() => setData({ ...data, housing: data.housing === h ? "" : h })}
                      className={`font-work-sans rounded-lg border px-4 py-2 text-sm font-normal transition ${data.housing === h ? "border-(--accent) bg-(--accent) font-semibold text-white" : "border-(--border) bg-gray-50 text-(--muted) hover:bg-gray-100"
                        }`}
                    >
                      {h}
                    </button>
              ...
              <div>
                <label className="font-work-sans mb-2 block text-sm font-medium text-(--muted)">Household Income</label>
                <div className="flex flex-wrap gap-2">
                  {INCOME.map((i) => (
                    <button
                      key={i}
                      onClick={() => setData({ ...data, income: data.income === i ? "" : i })}
                      className={`font-work-sans rounded-lg border px-4 py-2 text-sm font-normal transition ${data.income === i ? "border-(--accent) bg-(--accent) font-semibold text-white" : "border-(--border) bg-gray-50 text-(--muted) hover:bg-gray-100"
                        }`}
                    >
                      {i}
                    </button>
        ...
        {step === 3 && (
          <div className="space-y-6">
            <h2 className="font-limelight text-2xl font-bold text-foreground tracking-tight">What issues matter to you?</h2>
              ...
              {ISSUES.map((issue) => (
                <button
                  ...
                  className={`font-work-sans rounded-xl border px-4 py-2 text-sm font-medium transition ${data.issues.includes(issue)
                    ? "border-(--accent-mid) bg-(--accent-mid) text-white shadow-md"
        ...
        {step === 4 && (
          <div className="space-y-6">
            <h2 className="font-limelight text-2xl font-bold text-foreground tracking-tight">What is your age?</h2>
            <div className="flex flex-wrap gap-2">
              {AGE.map((a) => (
                <button
                  key={a}
                  onClick={() => setData({ ...data, age: data.age === a ? "" : a })}
                  className={`font-work-sans rounded-lg border px-4 py-2 text-sm font-normal transition ${data.age === a ? "border-(--accent) bg-(--accent) font-semibold text-white shadow-md" : "border-(--border) bg-gray-50 text-(--muted) hover:bg-gray-100"
                    }`}
                >
                  {a}
                </button>
        ...
        {step === 5 && (
          <div className="space-y-6">
            <h2 className="font-limelight text-2xl font-bold text-foreground tracking-tight">Any demographics apply?</h2>
            <p className="font-work-sans text-sm font-normal text-(--muted)">Select all that apply.</p>
            <div className="flex flex-wrap gap-2 max-h-[35vh] overflow-y-auto pr-2 pb-4 scrollbar-thin">
        ...
        <div className="mt-10 flex items-center justify-between border-t border-(--border) pt-5">
          {step < 5 ? (
            <button onClick={handleNext} className="font-work-sans text-sm font-medium text-(--muted) hover:text-foreground transition">
              Skip Question
            </button>
          ) : (
          ...
          {step < 5 ? (
            <button onClick={handleNext} className="font-work-sans rounded-xl bg-(--accent) px-6 py-2.5 text-sm font-semibold text-white shadow-md transition hover:bg-(--accent-mid)">
              Next
            </button>
          ) : (
            <button onClick={handleFinish} className="font-work-sans rounded-xl bg-(--accent-mid) px-6 py-2.5 text-sm font-semibold text-white shadow-md transition hover:scale-105">
              Complete Profile
            </button>
          )}
        </div>
