...
import { timeAgo, srcColor } from "@/lib/policyMetadata";
import { getRecentPolicies, type PolicyBriefing } from "@/lib/api";
import { getAreaMetas } from "./DigestCards";
...
export function RecentUpdates({ context, isPersonalized, profile }: Props) {
  ...
  const fetchItems = async () => {
    ...
    setError(null);
    try {
      const { policies } = await getRecentPolicies(borough, area, days, 50);
      setItems(policies);
    } catch (e) {
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="min-w-0 flex-1">
                    {/* source type badge + issue area tags */}
                    <div className="flex items-center gap-1.5 flex-wrap mb-1.5">
                      ...
                      {areas.map(area => (
                        <span key={area.id}
                          ...
                          style={{ color: area.color, background: `${area.color}12`, borderColor: `${area.color}30` }}>
