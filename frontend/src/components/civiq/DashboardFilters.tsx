...
const TRIGGER =
  "filter-dd-trigger font-work-sans flex w-full min-w-0 cursor-pointer items-center justify-between gap-2 rounded-xl border border-[var(--border)] bg-white/80 dark:bg-[var(--surface-elevated)]/80 px-3 h-[36px] text-left text-[11px] font-bold text-[var(--foreground)] shadow-sm backdrop-blur-md outline-none transition hover:bg-slate-50 dark:hover:bg-[var(--surface-elevated)] focus-visible:ring-2 focus-visible:ring-[var(--accent-soft)]";

const PANEL =
  "filter-dd-panel absolute left-0 right-0 z-[70] mt-1 max-h-60 overflow-auto rounded-xl border border-[var(--border)] bg-white/95 dark:bg-[var(--surface-card)]/95 py-1 shadow-lg backdrop-blur-xl";
  ...
  return (
    <div ref={rootRef} className="relative min-w-0">
      <button ref={triggerRef} type="button" aria-haspopup="listbox" aria-expanded={open}
        ...
        <ChevronDown className={`pointer-events-none size-3.5 shrink-0 text-[var(--muted)] transition-transform duration-200 dark:text-[var(--icon-violet)] ${open ? "rotate-180" : ""}`} strokeWidth={2} aria-hidden />
      </button>
      {open && (
        <ul id={`${instanceId}-lb`} role="listbox" className={PANEL}>
          {options.map((opt) => (
            <li key={opt} role="none" className="px-1">
              <button type="button" role="option" aria-selected={value === opt}
                className={`filter-dd-option font-work-sans flex w-full rounded-lg px-2.5 py-1.5 text-left text-[11px] font-bold transition ${value === opt ? "bg-[var(--accent)]/10 text-[var(--accent)] dark:bg-[var(--surface-elevated)] dark:text-[var(--accent-soft)]" : "text-[var(--foreground)] hover:bg-slate-100 dark:hover:bg-[var(--surface-elevated)]/70"}`}
...
export function DashboardFilters({
  ...
  return (
    <div className="relative z-40 w-full">
      <div className="glass-card surface-float soft-inset rounded-3xl px-5 py-4 border border-[var(--border)] bg-white/60 dark:bg-[var(--surface-card)]/50 backdrop-blur-md shadow-sm flex flex-col gap-4">

        {/* Dropdowns and Control Toggles */}
        <div className="flex flex-col lg:flex-row lg:items-end gap-4 lg:gap-24">

          {/* Map Mode control */}
          <div className="flex flex-col gap-1.5 flex-1 lg:max-w-[200px]">
            <span className="font-work-sans text-[10px] font-bold uppercase tracking-widest text-[var(--muted)]">Map Mode</span>
            <div className="flex items-center rounded-xl border border-[var(--border)] bg-slate-100/80 dark:bg-[var(--surface-elevated)]/60 p-0.5 shadow-inner gap-0.5 h-[36px] w-full">
              <button
                type="button"
                onClick={() => setMapMode("vector")}
                className={`flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all h-full flex-1 ${mapMode === "vector"
              ...
              <button
                ...
                className={`flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all h-full flex-1 ${mapMode === "satellite"
          ...
          {/* Location Dropdown */}
          <div className="flex flex-col gap-1.5 flex-1 lg:max-w-[200px]">
            <span className="font-work-sans text-[10px] font-bold uppercase tracking-widest text-[var(--muted)]">Location</span>
          ...
          <div className="flex flex-col gap-1.5 flex-1 lg:max-w-[180px]">
            <span className="font-work-sans text-[10px] font-bold uppercase tracking-widest text-[var(--muted)]">Timeframe</span>
          ...
          <div className="flex flex-col gap-1.5 flex-1 lg:max-w-[320px]">
            <div className="flex items-center gap-1.5">
              <span className="font-work-sans text-[10px] font-bold uppercase tracking-widest text-[var(--muted)]">Perspective</span>
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center rounded-xl border border-[var(--border)] bg-slate-100/80 dark:bg-[var(--surface-elevated)]/60 p-0.5 shadow-inner gap-0.5 h-[36px] flex-1">
                <button
                  type="button"
                  onClick={() => setIsPersonalized(true)}
                  className={`flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all h-full flex-1 ${isPersonalized
                    ? "bg-[var(--accent)] text-white shadow-sm"
                    : "text-[var(--muted)] hover:text-[var(--foreground)]"
                    }`}
                >
              ...
              <button
                ...
                className="flex items-center justify-center rounded-xl border border-[var(--border)] bg-white/80 dark:bg-[var(--surface-elevated)]/80 px-2.5 h-[36px] text-[var(--muted)] hover:text-[var(--foreground)] transition-all hover:bg-slate-50 dark:hover:bg-[var(--surface-elevated)] shadow-sm active:scale-[0.98] shrink-0"
