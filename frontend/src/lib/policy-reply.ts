...
/**
 * Dedupe and cap `key_numbers` for the KPI strip. Grounding (rejecting placeholders
 * like `$X` and figures the model invented) is enforced upstream by the backend's
 * `filter_key_numbers_to_context`, so the frontend only needs hygiene.
 */
export function buildDisplayKeyNumbers(
  ...
  const out: string[] = [];
  for (const raw of b.key_numbers) {
    const t = raw.trim();
    if (!t) continue;
...
export function normalizePolicyReply(payload: unknown): PolicyResponse {
  ...
  const key_numbers = filterStrings(p.key_numbers).map((s) => s.trim()).filter(Boolean);
